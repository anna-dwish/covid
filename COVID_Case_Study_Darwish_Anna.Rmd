...
Because of the interaction term between age and race/ethnic identity, it is slightly more complex to interpret these terms. We will accomplish this by considering NC residents of the same racial/ethnic identity. Note that the race/ethnic identity terms in Figure 4 depict the *combined* estimate of the main effect terms and the age interaction terms. 
...
mu.params <- c("alpha", 
               ...
               "beta9", 
               "beta10")
mnar.final.model.forest.table <- mnar.final.model.df[mu.params, ]
```

```{r mnar vs mcar forest plot, fig.align="center"}
names(mnar.final.model.forest.table)[1] <- "Estimate"
names(mnar.final.model.forest.table)[2] <- "Std.Error"
mnar.final.model.forest.table$`P-Value` <- NA_integer_
mnar.final.model.forest.table$Variables <- rownames(mnar.final.model.forest.table)
mnar.final.model.forest.table <- mnar.final.model.forest.table %>% 
  dplyr::select("Variables", 
                "Estimate", 
                "Std.Error", 
                "P-Value")

rownames(mnar.final.model.forest.table) <- c("Intercept",
                                  "Gender = Male",
                                  "Race/Ethnicity = Asian",
                                  "Race/Ethnicity = Black",
                                  "Race/Ethnicity = Hispanic/Latino",
...
mnar.final.model.forest.table <- generate_forest_plot_df(mnar.final.model.forest.table)
```
...
generate_model_agreement_df <- function(df, baseline.df, merge.df){
  df.vars <- rownames(df)
  df.agreement <- (df$Estimate >= baseline.df[df.vars,]$Estimate - 1.96*baseline.df[df.vars,]$Std.Error &
    df$Estimate <= baseline.df[df.vars,]$Estimate + 1.96*baseline.df[df.vars,]$Std.Error) | 
    (baseline.df[df.vars,]$Estimate >= df$Estimate - 1.96*df$Std.Error &
    baseline.df[df.vars,]$Estimate <= df$Estimate + 1.96*df$Std.Error) |
    (0 >= baseline.df[df.vars,]$Estimate- 1.96*baseline.df[df.vars,]$Std.Error &
    0 <= baseline.df[df.vars,]$Estimate + 1.96*baseline.df[df.vars,]$Std.Error & 
      0 >= df$Estimate- 1.96*df$Std.Error &
    0 <= df$Estimate + 1.96*df$Std.Error)
...
model.vs.baseline.list <- list(list(sensitivity.forest.table, pres.trump.forest.table),
                           list(mnar.final.model.forest.table, mcar.model.mice.estimates.unweighted.forest.table),
                           list(county.mod.forest.table, pres.trump.forest.table))

model.vs.baseline.results <- lapply(model.vs.baseline.list, 
                                    function(x) generate_model_agreement_df(df=x[[1]],
                                                                            baseline.df=x[[2]],
                                                                            merge.df=pres.trump.forest.table)) %>% 
  bind_cols() %>%
...
rownames(model.vs.baseline.results.matrix) <- c("Asian",
                                                "Black",
                                                "Hispanic/Latino",
                                                "Other",
                                                "White",
