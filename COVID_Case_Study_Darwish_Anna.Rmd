...
forestplot(rownames(mnar.final.model.forest.table), 
           ...
           col = fpColors(box = c("blue", "darkred"),
                          line = c("blue", "darkred")),
           legend = c("Final", "MNAR"),
           title="Figure INSERT: Final Model vs MNAR Model",
           txt_gp = fpTxtGp(title= gpar(fontfamily = "serif", fontface="plain", fontsize=10),
                            label = gpar(fontfamily = "serif", fontface="plain", fontsize=10),
                            ticks = gpar(fontfamily = "serif"),
                            xlab  = gpar(fontfamily = "serif")))
...
tidybayes::gather_draws(mnar.final.model %>% as.mcmc(.),
  ...
  labs(title = "Traceplots of a and b", 
...
generate_density_plot <- function(prior_meta,
                                  param,
                                  final_model=F){
  if(final_model){
    rjags_model <- mnar.final.model
    prior_lab <- expression(beta~" ~ N(0,1/10), a ~ N(0,1/20), b ~ N(0,1/20)")
    line_type = "solid"
  }
  else{
  ...
  p <- rjags_model$BUGSoutput$sims.matrix %>% 
    ...
    ggplot() + 
    geom_density(aes(x = (!!as.symbol(param)))) + 
    geom_vline(xintercept = mnar.final.model.df[param, "mean"], 
               color = "red",
               linetype=line_type) + 
    labs(x=prior_lab) +
    ylab("") +
...
priors <- list(list("beta_0_15-a_0_15-b_0_15/full-intrxn-no-health-beta_0_15-a_0_15-b_0_15.Rdata",
                    expression(beta~" ~ N(0,1/15), a ~ N(0,1/15), b ~ N(0,1/15)")),
               list("beta_0_20-a_0_20-b_0_20/full-intrxn-no-health-alpha_0_1-beta_0_20-a_0_20-b_0_20.Rdata",
                    expression(beta~" ~ N(0,1/20), a ~ N(0,1/20), b ~ N(0,1/20)")),
               list("beta_0_25-a_0_25-b_0_25/full-intrxn-no-health-beta_0_25-a_0_25-b_0_25.Rdata",
                    expression(beta~" ~ N(0,1/25), a ~ N(0,1/25), b ~ N(0,1/25)")))
```

```{r a sensitivity, cache=T}
a_plots <- lapply(priors, function(x) generate_density_plot(x, "a"))
a_plots[[4]] <- generate_density_plot("", 
                                      "a",
                                      final_model = T)
```

```{r a sensitivity plots display, fig.align="center", fig.width=6, fig.height=3}
do.call("grid.arrange", 
        c(a_plots, 
          ncol=2,
          top="Figure INSERT: Prior Sensitivity ~ a"))
```

```{r b sensitivity, cache=T}
b_plots <- lapply(priors, function(x) generate_density_plot(x, "b"))
b_plots[[4]] <- generate_density_plot("", 
                                      "b",
                                      final_model = T)
```

```{r b sensitivity plots display, fig.align="center", fig.width=6, fig.height=3}
do.call("grid.arrange", 
        c(b_plots, 
          ncol=2,
          top="Figure INSERT: Prior Sensitivity ~ b"))
```

With respect to traceplots, random scatter around the estimate and clear "mixing" between different chains indicate convergence. An ideal autocorrelation plot will peak at the beginning and quickly drop close to 0, which provides evidence that the iterations sufficiently explored the parameter space and weren't strongly dependent on one another. $\hat{R}$ values provide an estimate of convergence based on the variance of an estimated parameter between chains, and the variance within a chain. An $\hat{R}$ value close to 1 suggests convergence. Finally, the $n_{e\mathit{ff}}$ provides insight into whether our model has achieved stationarity, as it estimates the number of independent observations we obtained in our posterior draws.
