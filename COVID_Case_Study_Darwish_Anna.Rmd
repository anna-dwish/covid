...
The following variables will be examined in this study. Please see Table 13 in the Appendix for more details on their meanings and values:
...
The following plots provide a brief overview of *Evaluated Social Distancing Adherence* over a few notable variables of interest: *President Trump Approval Score*, *Children in Household*, *Race/Ethnic Identity*, and *Age*. Please see Figure 19 in the Appendix for the remaining exploratory plots, and note that both of these figures were generated from one of many full imputations. The steps taken to develop these imputations is discussed in the *Methods* section.
...
$$
\begin{aligned}
log(\frac{M_{i}}{1-M_{i}}) = \alpha + \beta_1 * I(Sex_i = Male) +\sum_{r=1}^4 \beta_{2r} * I(Race.Ethnicity_i = r) \ + \\
 \sum_{r=0}^4 \beta_{3r} * Age_i * I(Race.Ethnicity_i = r)
 \\
 log(\frac{P_{i}}{1-P_{i}}) = a + b * y_i
 \\
 y_i \sim Bern(M_i), \ \ \alpha \sim N(0, 1), \ \ \beta_i \sim N(0,0.1), \ \ a \sim N(0,0.05) , \ \ b \sim N(0,0.05)
\end{aligned}
$$
...
demographics.only.fit.results <- cv_plot(with.age.children.race.gender.college.house.intrxn.age.race,
                       ...
                       covid.df.regression,
                       create.cm.plot=TRUE,
                       subtitle.cm = "Figure 10: Demographics-Only Model on Imputation 1")
```
...
tidybayes::gather_draws(mnar.final.model %>% as.mcmc(.),
  ...
  labs(title = "Figure 5: Traceplots of a and b", 
       x = "", y = "", color = "Chain") +
...
Figure 5 displays traceplots for our final model's *a* and *b* parameter chains. The random scatter around the mean estimate, along with clear "mixing" between the different chains, indicate convergence for these parameters. Please reference Figure 14 in the Appendix for the remaining traceplots for our final model.

Figure 15 in the Appendix contains our auto-correlation plots for our *a* and *b* parameters, where we see a peak at the beginning that quickly drops close to 0 as the lag increases. This provides evidence that the iterations sufficiently explored the parameter space, as they were not strongly dependent upon one another.

Table 5 summarizes our $\hat{R}$ and $n_{e\mathit{ff}}$ values across all of the parameters and covariates. An $\hat{R}$ value between 1 and 1.05 suggests convergence, which holds true for all of the table entries. Finally, the $n_{e\mathit{ff}}$ provides insight into whether our model has achieved stationarity, as it estimates the number of independent observations we obtained in our posterior draws. An arbitrary cutoff for acceptable $n_{e\mathit{ff}}$ values is between 150 and 200, which each of our parameters also satisfy.
...
priors <- list(list("beta_0_10-a_0_20-b_0_20/full-intrxn-no-health-alpha_0_1-beta_0_10-a_0_20-b_0_20.Rdata",
               ...
               list("beta_0_25-a_0_25-b_0_25/full-intrxn-no-health-beta_0_25-a_0_25-b_0_25.Rdata",
                    " ~ N(0,1/25); a,b ~ N(0,1/25)"))
```
...
```{r b sensitivity plots display, fig.align="center", fig.width=9, fig.height=3.5}
b_prior_plot <- b_df  %>% 
  ggplot() + 
...
forestplot(rownames(mnar.final.model.forest.table), 
           ...
           col = fpColors(box = c("blue", "darkred"),
                          line = c("blue", "darkred")),
           legend = c("Final", "MNAR"),
           title="Figure 7: Final Model vs MNAR Model",
           txt_gp = fpTxtGp(title= gpar(fontfamily = "serif", fontface="plain", fontsize=10),
                            label = gpar(fontfamily = "serif", fontface="plain", fontsize=10),
                            ticks = gpar(fontfamily = "serif"),
                            xlab  = gpar(fontfamily = "serif")))
...
Figure 7 summarizes a forestplot comparing the MNAR model's estimates to the one generated using the same covariates (age, gender, and race/ethnic identity) with the first modeling approach. This highlights key disagreements among all of the racial/ethnic estimates, except for those who identified as "Other". The strongest points of disagreements were the combined effect of Age and being White and that of Age and being Black.

There are multiple reasons for this discrepancy. Firstly, in parameterizing the second level of MNAR model, it isn't suprising that our demographic covariates would converge to slightly different values. Further, the poor representation among Asian, Hispanic/Latino, and those who identified as "Other" compounded instability in both modeling approaches. Another insight this discrepancy offers is that the effects of race/ethnicity on social distancing adherence are quite complex and require information from other available covariates to better understand the response.

For example, as we can see in Table 6 below, the distribution of President Trump Approval Score, for those participants with an available value, is highly dependent on the race/ethnic identity of the North Carolinian resident. Given the limitation that our MNAR approach only handled fully populated covariates, we can not draw any specific conclusions from the variable estimates relative to our final model.
...
covid.df %>%
  ...
  dplyr::summarize(`Mean Score` = round(mean(trump_approve_score, 
                                                na.rm=T),2),
                   .groups="drop") %>%
  pander(., "President Trump Approval Score Summmary Across Racial/Ethnic Identities")
```
...
**DISCUSSION OF a AND b - Discuss tomorrow**
...
county.cm <- plot_confusion_matrix(county.cm.tibble, 
                              add_row_percentages = FALSE,
                              add_col_percentages = FALSE) + 
  ggplot2::labs(subtitle = "Figure 17: Random County Intercept Model") + 
  ggplot2::theme(plot.subtitle = element_text(hjust = 0.5, size = 10))
...
forestplot(rownames(pres.trump.forest.table[county.row, ]), 
           ...
           title="Figure 8: Final Model vs County Intercept Model",
           txt_gp = fpTxtGp(title= gpar(fontfamily = "serif", fontface="plain",fontsize=10),
                            label = gpar(fontfamily = "serif", fontface="plain",fontsize=10),
...
arm::binnedplot(x=covid.df.demographic.preds,y=residuals(pooled.predict.demographic.obj, "pearson", scaled = TRUE),
                ...
                main = "Figure 11: Demographics Binned Residuals",
...
arm::binnedplot(x=covid.df.trump.preds,y=residuals(pooled.predict.trump.obj, "pearson", scaled = TRUE),
                xlab="Predicted Probabilities", 
                ylab="Binned Residuals",
                main = "Figure 12: President Trump Binned Residuals",
                cex.main=0.9, cex.lab=0.8)
```
...
forestplot(rownames(pres.trump.forest.table), 
           ...
           title="Figure 13: Final Model vs Complete Case Model",
           txt_gp = fpTxtGp(title= gpar(fontfamily = "serif", fontface="plain", fontsize=10),
                            label = gpar(fontfamily = "serif", fontface="plain", fontsize=10),
                            ticks = gpar(fontfamily = "serif"),
                            xlab  = gpar(fontfamily = "serif")))
...
```{r remaining traceplots, fig.align="center", fig.height=5}
tidybayes::gather_draws(mnar.final.model %>% as.mcmc(.), 
                        alpha, 
                        beta1, 
                        beta2, 
  ...
  labs(title ="Figures 14 and 15: Traceplots and ACF Plots", 
       x = "Iteration", 
       y = "Value", 
       color = "Chain") +
  theme(legend.position="none")
...
arm::binnedplot(x=rand.county.predictions,y=residuals(covid.df.county.mod, "pearson", scaled = TRUE),
                xlab="Predicted Probabilities", 
                ylab="Binned Residuals",
                main = "Figure 16: Random County Binned Residuals",
                cex.main=0.9, cex.lab=0.8)
```
...
random.effects.plot <- plot_model(covid.df.county.mod, type = "re", facet.grid=FALSE, title="Figure 18: Random Effects over Counties") 
