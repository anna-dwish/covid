...
Some of the questions asked included the number of people the respondent had face-to-face interactions with outside of their household in the past 24 hours, whether they maintained at least 6 feet of distance in these interactions, general demographic information, and the respondents' perceptions of how other North Carolinians were responding to COVID-19. This was augmented with internal databases from Clarity Campaigns, which mapped respondents' demographic information (age, gender, race/ethnic identity, college degree status, party affiliation, etc.) to their expected opinions on various political topics, such as their approval of President Trump.
...
Figure 1 below shows the distribution of weights across the different race/ethnic identities and age. While all race/ethnic identities have high outliers, Asian and Hispanic/Latino residents had a notably higher median weight relative to other identities, reflecting lower participation rates in general. Unsurprisingly, younger residents, particularly those under about 35, tended to have a higher weight than older residents, reflecting lower participation rates in younger age groups.
...
```{r weight distribution, fig.width=9, fig.height=4}
race.ethnicity.weight <- ggplot(covid.df, 
       aes(y=weight,
           x=Q19.20..Race...Ethnicity,
           color=Q19.20..Race...Ethnicity)) +
  geom_boxplot() +
  labs(x="Race/Ethnic Identity",
       y="Survey Weight",
       color="Race/Ethnic Identity") +
  theme(legend.direction="horizontal")

age.weight <- ggplot(covid.df, aes(x=age, 
                                   y=weight, 
                                   color=Q19.20..Race...Ethnicity)) + 
  geom_point() +
  labs(x="Age",
...
```{r}
covid.df %>% filter(Social.Distancing.Adherence==FALSE & Q13..Currently.Practicing.Social.Distancing.=="Yes") %>%
  dplyr::select(Q6..Non.HH.Face.to.Face.Count,
                 Q7..Six.Feet.Away...If.Q6...0.,
                Q10..Times.in.Group...20.in.Last.Week) %>%
  mutate(Reason=case_when(
    Q6..Non.HH.Face.to.Face.Count > Q7..Six.Feet.Away...If.Q6...0. ~ "Non-Socially Distanced Encounter(s)",
    Q10..Times.in.Group...20.in.Last.Week > 0 ~ "Large Gathering (+20)",
    TRUE ~ "Breakoff"
  )) %>%
  dplyr::group_by(Reason) %>%
  dplyr::summarize(Count=n(), 
                   .groups="drop") %>%
  pander(., "Reason for Disagreement between SSDA and ESDA")
```


## Predictor Variables

The following variables will be examined in this study. Please see Table 14 in the Appendix for more details on their meanings and values:
...
The following plots provide a brief overview of *Evaluated Social Distancing Adherence* over a few notable variables of interest: *President Trump Approval Score*, *Children in Household*, *Race/Ethnic Identity*, and *Age*. Please see Figure 23 in the Appendix for the remaining exploratory plots, and note that both of these figures were generated from one of many full imputations. The steps taken to develop these imputations is discussed in the *Methods* section.

Firstly, from the top left plot, there does not appear to be significant differences in *ESDA* across different levels of approval scores of President Trump. Secondly, we can see in the upper-right plot that either participants with no children in their household or 3+ children in their household were slightly less likely to be socially distancing. While it makes sense that larger households would struggle more to adhere to guidelines, it is difficult to explain the case of 0 children as this encompasses a wide variety of living situations [[15]][Bibliography]. From the lower-left plot, it appears as though Asian identifying participants had a higher rate of *ESDA*, while Black participants and those identifying as "Other" had a slightly lower rate of *ESDA*. An important point to keep in mind here is that Black and Indigenous people are much more likely to be front-line workers[[15]][Bibliography], which makes social distancing quite challenging. Finally, it appears as though the median age of participants that were adhering to social distancing rules were slightly younger than those who were not. However, these two barplots are largely the same.

# Methods

## Missing Data

To ensure independence among observations, I took the most recent survey submission for each unique participant in this dataset. Given that it took time for many people to understand the gravity of COVID-19, compounded with conflicting information on best practices, this decision relies on the assumption that later surveys are more representative of residents' current behaviors. Additionally, every model, except the MNAR model, accounts for the sampling weight associated with each participant as inversely proportional to the dispersions.

The first step in our methodology will be addressing the missing data. In order to fill in the missing predictor variables, I used a multiple imputation approach on the available demographic data with the *mice* package. This uses multiple linear and logistic regression to populate variables with missing data. An unfortunate limitation with this approach is that it makes the assumption that the data is missing at random, conditional on other measured data. This could be a stretch as it appears to be missing not at random, which was discussed in the *Missing Data Overview & Handling* section. However, we can examine the reliability of these estimates with sensitivity analysis [[16]][Bibliography]. By conducting multiple iterations of imputation, it is possible to understand the uncertainty with regards to our imputation method. Per the standard rule of using the percentage of missing data as the number of iterations as percentage of missing data, I conducted 25 iterations of missing data imputation. Note that all aforementioned exploratory analysis plots (Figures 2 and 23) stem from the first imputation.

For each of the 25 imputations, I began with a full model of all demographic covariates, excluding the President Trump approval score. As a reminder, these were: *Age*, *Gender*, *Number of People in Household*, *Children in Household*, *College Degree*, and *Race/Ethnicity*. Using the *glmnet* package, I conducted lasso regression to determine which covariates were deemed significant for that particular imputation. Table 4 shows each of these main-effect only models, along with the number of imputations that yielded that model. Note that this step was strictly used to determine which main effects appear to be most useful and did not determine the final model. All of of these models were some combination of main effects from *Age*, *Children in Household* (labeled as Children), and *College Degree*.
...
In addition to the sensitivity analysis involved in creating models across 25 different imputations, I also recreated the final President Trump Approval Score model using the *mice* package to impute values for our response prior to incorporating the breakoff point assumption. This is a classical MAR approach, as we rely solely upon the available co-variates to populate the response values. In order to establish a baseline for expected performance, I also recreated this final model via a complete case approach. The coefficient estimates of the final model, the sensitivity model, and a complete case approach model are located in Tables 10, 11, and 12, respectively. Random forest plots comparing the coefficient estimates from the latter two models to those of the final one are located in Figures 5 and 17, respectively.

While the final model incorporates missingness information directly into the response, it relies on a strong assumption with respect to the nature of the missingness. For this reason, I also created a non-ignorable missing data selection model, where I first use the complete demographic covariates (age, race/ethnicity, and gender) to predict the likelihood the participant is adhering to social distancing rules, per the definition of *ESDA*. The next level then relates that likelihood to the likelihood that the surveyee's *ESDA* response is missing. 

While prior selection built on previous literature is often a key component of Bayesian modeling, the novelty of pandemic behaviors did not sufficiently align with existing literature on other official health recommendations, such as wearing a seatbelt or getting the flu vaccine. For this reason, our final model's prior, along with those used for prior sensitivity analysis, are relatively non-informative. Note that the variance of the priors are relatively low due to the scale of the response. The *Results* section contains a more thorough comparison of the other priors.

The parameterization of this model is:

$$
\begin{aligned}
log(\frac{P_{i}}{1-P_{i}}) = \alpha + \beta_1 * I(Sex_i = Male) +\sum_{r=1}^4 \beta_{2r} * I(Race.Ethnicity_i = r) \ + \\
 \sum_{r=0}^4 \beta_{3r} * Age_i * I(Race.Ethnicity_i = r)
 \\
 log(\frac{M_{i}}{1-M_{i}}) = a + b * y_i
 \\
 y_i \sim Bern(P_i), \ \ \alpha \sim N(0, 1), \ \ \beta_i \sim N(0,1/5), \ \ a \sim N(0,1/15) , \ \ b \sim N(0,1/15)
\end{aligned}
$$
...
Estimates in this model were unwieldy due to limited representation from non-White race/ethnic groups. In order to ensure convergence, I examined traceplots and autocorrelation plots for each of the parameters, along with their $\hat{R}$ and $n_{e\mathit{ff}}$ values. Prior sensitivity was also conducted via inspection of the posterior distribution of our parameters under different prior assumptions. The ideal results of these posterior checks are more fully discussed in the *Results* section.
...
One significant drawback to this model is that county is missing from 1710 participants, and missing data imputation isn't sensible given there are 100 counties in North Carolina. Additionally, because Rubin's rules do not apply well to mixed effects models, I opted for a complete case approach to generate this model. In order to ensure all counties were represented at least once in the dataset, this model was limited in the number of covariates we could use without losing all observations for rare counties. For this reason, the final county model's covariates reflects the second best performance among the demographic models explored (see Table 5). This yielded the following model, where $\alpha_j$ is the associated intercept for county $j$:

$$ 
\begin{aligned}
log(\frac{P_{ij}}{1-P_{ij}}) = \alpha_0 + \alpha_j + \beta_{1} * I(Sex_{ij}=Male) + \beta_{2} * Children_{ij} + \beta_3 * President.Trump_{ij} \ + \\
\sum_{r=1}^4 \beta_{4r} * I(Race.Ethnicity_{ij} = r) + \sum_{r=0}^4 \beta_{5r} * Age_{ij} * I(Race.Ethnicity_{ij} = r) \\
...
Table 5 summarizes the different models I explored for the demographics-only model, their F1-scores, and false positive rates. All of these models were predisposed to committing a false positive, and our final demographics-only model performed best in terms of both metrics. 

From the confusion matrix in Figure 13 in the Appendix, we see an example of this high false positive rate. However, the residual plot in Figure 14 in the Appendix shows a relatively consistent random scatter about the zero line. The limitations to this model's predictive performance are likely a consequence of the fact that the ability and/or decision to socially distance varies largely from person to person. 

```{r performance evaluation, warning=FALSE, cache=T}
...
fit.results.trump <- cv_plot(with.age.children.race.gender.college.house.trump.intrxn.age.race,
                       ...
                       covid.df.regression,
                       create.cm.plot=TRUE,
                       subtitle.cm = "Figure 3: President Trump Approval Score Model")
fit.results.trump[[3]]
```
...
From Table 10 in the Appendix, we will begin by interpreting the demographic main effects. Our model predicts that for every additional adult household member, we are 95% confident that the odds a participant is social distancing multiplies by a factor between `r round(exp(pres.trump.table["Household Size","Estimate"]- 1.96 * pres.trump.table["Household Size","Std.Error"]),3)` and `r round(exp(pres.trump.table["Household Size","Estimate"]+ 1.96 * pres.trump.table["Household Size","Std.Error"]),3)`, holding all else constant. It is important to note here that the logistical implications for what this variable means is highly variable from household to household. 

With respect to the *Children* variable, it is awkward to interpret this term directly given the presence of the *Household Size* variable (if we hold all else constant, we are essentially considering the situation of swapping an adult member of the household with a child member). For this specific scenario, we are 95% confident that the odds a participant is socially distancing multiplies by a factor between `r round(exp(pres.trump.table["Children in Household","Estimate"]- 1.96 * pres.trump.table["Children in Household","Std.Error"]),3)` and `r round(exp(pres.trump.table["Children in Household","Estimate"]+ 1.96 * pres.trump.table["Children in Household","Std.Error"]),3)` for every additional child in their household, assuming all else remains the same. If we assume that an additional child in the household also means the total household size increases, our model predicts that the odds a participant is socially distancing multiplies by a factor of `r round(exp(pres.trump.table["Children in Household","Estimate"] + pres.trump.table["Household Size","Estimate"]),3)` for an additional child. After accounting for all other co-variates, neither gender nor college degree status was significant.

```{r generate forest plot df functions, message=FALSE, fig.height=3.5, fig.align="center"}
generate_forest_plot_df <- function(df, shrink=10){
  df.forest <- df %>%
    subset(as.vector(is.na(str_match(rownames(.), 
                               "Race"))))
  ...
  # Scale main race/ethnic for visual purposes
  df.main.race <- df %>%
    subset(str_starts(rownames(.), "Race")) %>%
    mutate(Estimate=Estimate/shrink,
           Std.Error=Std.Error/shrink)
  rownames(df.main.race) <- word(rownames(df.main.race), 
                                 -1)
  
  # format rownames of race/ethnic and age intrxn terms
  df.intrxn.race <- as.data.frame(list(Variables="Age", 
                              Estimate=NA_integer_, 
                              Std.Error=NA_integer_, 
                              `P-Value`=NA_integer_))
  rownames(df.intrxn.race) <- "Age"
  colnames(df.intrxn.race) <- colnames(df)
  df.intrxn.race <- rbind(df.intrxn.race, df %>% 
                            subset(str_starts(rownames(.), "Age")))
  rownames(df.intrxn.race) <- word(rownames(df.intrxn.race), -1)
  rownames(df.intrxn.race) <- paste0(c("", rep("--", nrow(df.intrxn.race)-1)), 
                                     rownames(df.intrxn.race))
...
forestplot(rownames(pres.trump.forest.table), 
           mean  = pres.trump.forest.table$Estimate, 
           lower = pres.trump.forest.table$Estimate - 1.96*pres.trump.forest.table$Std.Error,
           upper = pres.trump.forest.table$Estimate + 1.96*pres.trump.forest.table$Std.Error,
           line.margin = .2,
           col = fpColors(box = c("blue"),
                          line = c("blue")),
           title="Figure 4: President Trump Approval Score Model",
...
Prior to the sensitivity analysis section, note that Figure 15 in the Appendix depicts a forestplot comparing this model to one that accounts for the survey mode (IVR vs Live). While survey mode was slightly significant, there was no improvement in the predictive power of that model nor did any of the other estimated terms notably change. Thus to maintain interpretability for all demographic groups, irrespective of the mode in which they received the survey, we ultimately chose not to incorporate it.

## Sensitivity Analysis

### Breakoff Assumption

As discussed in the *Response Variable* section, *ESDA* was generated from a combination of answers pertinent to social distancing and the breakoff point of the survey if that information was unavailable. To conduct sensitivity analysis to this decision, the following model was generated by using the *mice* package to impute *ESDA*. In this manner, no assumptions were made regarding the breakoff point, and this approach reflects a classic MAR framework.
...
sensitivity.table <- sensitivity.table[,colnames(pres.trump.forest.table)]
rownames(sensitivity.table) <- sensitivity.table$Variables
```

```{r forestplot sensitivity model, message=FALSE, fig.height=4, fig.align="center"}
sensitivity.forest.table <- generate_forest_plot_df(sensitivity.table)
...
forestplot(rownames(pres.trump.forest.table), 
           ...
           col = fpColors(box = c("blue", "darkred"),
                          line = c("blue", "darkred")),
           legend = c("Final", "MAR"),
           title="Figure 5: Final Model vs MAR Model",
           txt_gp = fpTxtGp(title= gpar(fontfamily = "serif", fontface="plain", fontsize=10),
                            label = gpar(fontfamily = "serif", fontface="plain", fontsize=10),
                            ticks = gpar(fontfamily = "serif"),
                            xlab  = gpar(fontfamily = "serif")))
...
In Figure 5, we can see that our final model and the sensitivity model largely agree with respect to covariate estimates. The most notable difference is that College Degree is significant in the sensitivity model, but the point estimates are nearly captured in each other's 95% confidence intervals. Because the estimates and certainty for most of the covariates are largely consistent between these two models, this suggests that the breakoff assumption did not largely alter our final model. Note that in this forest plot, along with similar ones, the main *Race/Ethnicity* effects and *Intercept* are scaled down by a factor of 10 for visual purposes.
...
complete.case.table <- data.frame(summary(complete.case.mod)$coefficients) %>% 
...
```{r complete case approach results on non missing participants, warning=F,message=F, fig.align="center", fig.height=3}
...
The confusion matrices in Figure 6 summarize the performance of our final model using a complete case approach. For both the set of complete surveys (4293 surveys) and the first imputation (7579 surveys), this model almost fails to correctly classify any of the NC residents who weren't social distancing. This provides evidence that an MCAR (Missing Completely At Random) approach is insufficient for this dataset. A table summarizing the model's coefficient estimates, along with a forest plot comparing them to our final model, are located in the Appendix (Table 12 and Figure 17).

### Non-Ignorable Data Selection Model

```{r main approach version, warning=F}
with.age.race.gender.intrxn.age.race <- with(data=imp.covid.df,
                                             expr = glm(Social.Distancing.Adherence ~ Q19.20..Race...Ethnicity +
                                                          DEMOGRAPHICS...GENDER +
                                                          age_race_ethnicity_asian +
                                                          age_race_ethnicity_black +
...
pool.age.race.gender.intrxn.age.race <-
  pool(with.age.race.gender.intrxn.age.race)
mcar.model.mice.estimates.table <-
  data.frame(summary(pool.age.race.gender.intrxn.age.race)) %>% 
  dplyr::select("term", "estimate", "std.error", "p.value")
colnames(mcar.model.mice.estimates.table) <- c("Variables", "Estimate", "Std.Error", "P-Value")
mcar.model.mice.estimates.table$Variables <-c("Intercept",
                                 "Race/Ethnicity = Asian",
                                 "Race/Ethnicity = Black",
                                 "Race/Ethnicity = Hispanic/Latino",
                                 "Race/Ethnicity = Other",
                                 ...
                                 "Age x Race/Ethnicity = Other",
                                 "Age x Race/Ethnicity = White")
rownames(mcar.model.mice.estimates.table) <- mcar.model.mice.estimates.table$Variables
mcar.model.mice.estimates.forest.table <- generate_forest_plot_df(mcar.model.mice.estimates.table)
```

```{r main approach version unweighted, warning=F}
with.age.race.gender.intrxn.age.race.unweighted <- with(data=imp.covid.df,
                                             expr = glm(Social.Distancing.Adherence ~ Q19.20..Race...Ethnicity +
                                                          DEMOGRAPHICS...GENDER +
                                                          age_race_ethnicity_asian +
                                                          age_race_ethnicity_black +
...
pool.age.race.gender.intrxn.age.race.unweighted <-
  pool(with.age.race.gender.intrxn.age.race.unweighted)
mcar.model.mice.estimates.unweighted.table <-
  data.frame(summary(pool.age.race.gender.intrxn.age.race.unweighted)) %>% 
  dplyr::select("term", "estimate", "std.error", "p.value")
colnames(mcar.model.mice.estimates.unweighted.table) <- c("Variables", "Estimate", "Std.Error", "P-Value")
mcar.model.mice.estimates.unweighted.table$Variables <-c("Intercept",
                                 "Race/Ethnicity = Asian",
                                 "Race/Ethnicity = Black",
                                 "Race/Ethnicity = Hispanic/Latino",
                                 "Race/Ethnicity = Other",
                                 ...
                                 "Age x Race/Ethnicity = Other",
                                 "Age x Race/Ethnicity = White")
rownames(mcar.model.mice.estimates.unweighted.table) <- mcar.model.mice.estimates.unweighted.table$Variables
mcar.model.mice.estimates.unweighted.forest.table <- generate_forest_plot_df(mcar.model.mice.estimates.unweighted.table)
```

```{r load non ignorable model}
# Called rjags_model
load(file="beta_0_5-a_0_15-b_0_15/beta_0_5-a_0_15-b_0_15.Rdata")
mnar.final.model <- rjags_model

mnar.final.model.df <- mnar.final.model$BUGSoutput$summary %>% 
  as.data.frame() 
mu.params <- c("alpha", 
               "beta1", 
               "beta2", 
               ...
               "beta9", 
               "beta10")
mnar.final.model.table <- mnar.final.model.df[mu.params, ]
```

```{r mnar vs mcar forest plot, fig.align="center"}
names(mnar.final.model.table)[1] <- "Estimate"
names(mnar.final.model.table)[2] <- "Std.Error"
mnar.final.model.table$`P-Value` <- NA_integer_
mnar.final.model.table$Variables <- rownames(mnar.final.model.table)
mnar.final.model.table <- mnar.final.model.table %>% 
  dplyr::select("Variables", 
                "Estimate", 
                "Std.Error", 
                "P-Value")

rownames(mnar.final.model.table) <- c("Intercept",
                                  "Gender = Male",
                                  "Race/Ethnicity = Asian",
                                  "Race/Ethnicity = Black",
                                  "Race/Ethnicity = Hispanic/Latino",
...
mnar.final.model.forest.table <- generate_forest_plot_df(mnar.final.model.table)
```

```{r traceplot, fig.height=2, fig.align='center'}
tidybayes::gather_draws(mnar.final.model %>% as.mcmc(.),
                        a,
                        b) %>% 
  ungroup() %>% 
  ggplot() + 
  ...
  labs(title = "Figure 7: Traceplots of a and b", 
       x = "", y = "", color = "Chain") +
...
```{r rhat and neff}
mnar.final.model.df[c(mu.params,
                      "a",
                      "b"), ] %>% 
  dplyr::select(Rhat, n.eff) %>%
  mutate(Rhat=format(round(Rhat,2),
                     nsmall=2)) %>%
  t(.) %>%
  as.data.frame() %>%
  knitr::kable(., format="latex",
               ...
               col.names=c("$\\alpha$",
                           ...
                           "a",
                           "b"),
               caption="$\\hat{R}$ and $N_{e\\mathit{ff}}$ Values") %>%
  kableExtra::kable_styling(latex_options = c("hold_position"))
```

Figure 7 displays traceplots for our final model's *a* and *b* parameter chains. The random scatter around the mean estimate, along with clear "mixing" between the different chains, indicate convergence for these parameters. Please reference Figure 18 in the Appendix for the remaining traceplots for our final model.

Figure 19 in the Appendix contains our auto-correlation plots for our *a* and *b* parameters, where we see a peak at the beginning that quickly drops close to 0 as the lag increases. This provides evidence that the iterations sufficiently explored the parameter space, as they were not strongly dependent upon one another.

Table 6 summarizes our $\hat{R}$ and $n_{e\mathit{ff}}$ values across all of the parameters and covariates. An $\hat{R}$ value between 1 and 1.05 suggests convergence, which holds true for all of the table entries. Finally, the $n_{e\mathit{ff}}$ provides insight into whether our model has achieved stationarity, as it estimates the number of independent observations we obtained in our posterior draws. An arbitrary cutoff for acceptable $n_{e\mathit{ff}}$ values is between 150 and 200, which each of our parameters also satisfy.

```{r density plots over priors}
generate_density_df <- function(prior_meta,
                                  param,
                                  final_model=F){
  if(final_model){
    rjags_model <- mnar.final.model
    prior_lab <- " ~ N(0,1/5); a,b ~ N(0,1/15)"
    line_type = "solid"
  }
  else{
    prior_path <- prior_meta[[1]]
    prior_lab <- prior_meta[[2]]
    line_type = "dashed"
  
    load(prior_path)
  }
  
  p <- rjags_model$BUGSoutput$sims.matrix %>% 
    as.data.frame() %>%
    dplyr::select((!!as.symbol(param))) %>%
    mutate(prior_name=prior_lab,
           line_aes=line_type)
...
priors <- list(list("beta_0_10-a_0_20-b_0_20/beta_0_10-a_0_20-b_0_20.Rdata",
                    " ~ N(0,1/10); a,b ~ N(0,1/20)"),
               list("beta_0_15-a_0_15-b_0_15/beta_0_15-a_0_15-b_0_15.Rdata",
                    " ~ N(0,1/15); a,b ~ N(0,1/15)"),
               list("beta_0_20-a_0_20-b_0_20/beta_0_20-a_0_20-b_0_20.Rdata",
                    " ~ N(0,1/20); a,b ~ N(0,1/20)"))
```

```{r a sensitivity, cache=T}
a_df <- lapply(priors, function(x) generate_density_df(x, "a")) %>% bind_rows(.)
a_df <- rbind(a_df, generate_density_df("", 
                                      "a",
                                      final_model = T))
```

```{r a sensitivity plots display, fig.align="center", fig.width=5, fig.height=2.5}
a_prior_plot <- a_df  %>% 
  ggplot() + 
  geom_density(aes(x = a)) + 
  geom_vline(aes(xintercept = mnar.final.model.df["a", "mean"],
                 linetype=line_aes),
             color = "red") + 
  labs(title = "Figure 8",
       x = "a",
  ...
  facet_wrap(prior_name~.,  
             labeller=label_bquote(beta~.(prior_name)),
             scales = "free_y", 
             nrow = 2) + 
  theme(legend.position="none")
```

```{r b sensitivity, cache=T}
b_df <- lapply(priors, function(x) generate_density_df(x, "b")) %>% bind_rows(.)
b_df <- rbind(b_df, generate_density_df("", 
                                      "b",
                                      final_model = T))
```

```{r b sensitivity plots display, fig.align="center", fig.width=10, fig.height=3.5}
b_prior_plot <- b_df  %>% 
  ggplot() + 
  geom_density(aes(x = b)) + 
  geom_vline(aes(xintercept = mnar.final.model.df["b", "mean"],
                 linetype=line_aes),
             color = "red") + 
  labs(title = "",
       x = "b",
  ...
  facet_wrap(prior_name~.,  
             ...
             nrow = 2) + 
  theme(legend.position="none")
grid.arrange(arrangeGrob(a_prior_plot,
                         b_prior_plot,
                         nrow=1))
```

In Figure 8, we explore the posterior distributions of $a$ and $b$ across different priors on the demographic co-variates, along with $a$ and $b$ themselves. The red line represents our final model's mean estimates of these parameters, and the density plots with a dashed line were derived from the chains of our final model. Given that our posterior means align well with the peaks for other priors, we have sufficient evidence that our parameters are not sensitive to prior choice. Note that each model had the same prior, $N(0,1)$, on the intercept term $\alpha$.

Thus, our model predicts that the odds of an available *ESDA* response from an NC resident who is not socially distancing is between `r round(exp(mnar.final.model.df["b", "mean"] - 1.96 * mnar.final.model.df["b", "sd"]),3)` and 
`r round(exp(mnar.final.model.df["b", "mean"] + 1.96 * mnar.final.model.df["b", "sd"]),3)` times less than that of an NC resident who is socially distancing.

```{r, fig.align="center"}
forestplot(rownames(mnar.final.model.forest.table), 
           mean  = cbind(mcar.model.mice.estimates.unweighted.forest.table$Estimate,
                         mnar.final.model.forest.table$Estimate), 
           lower = cbind(mcar.model.mice.estimates.unweighted.forest.table$Estimate -
                           1.96*mcar.model.mice.estimates.unweighted.forest.table$Std.Error, 
                         mnar.final.model.forest.table$Estimate-
                           1.96*mnar.final.model.forest.table$Std.Error),
           upper = cbind(mcar.model.mice.estimates.unweighted.forest.table$Estimate +
                           1.96*mcar.model.mice.estimates.unweighted.forest.table$Std.Error,
                         mnar.final.model.forest.table$Estimate +
                           1.96*mnar.final.model.forest.table$Std.Error),
           line.margin = .2,
           col = fpColors(box = c("blue", "darkred"),
                          line = c("blue", "darkred")),
           legend = c("Mice", "MNAR"),
           title="Figure 9: Unweighted Mice Model vs MNAR Model",
           txt_gp = fpTxtGp(title= gpar(fontfamily = "serif", fontface="plain", fontsize=10),
                            label = gpar(fontfamily = "serif", fontface="plain", fontsize=10),
                            ticks = gpar(fontfamily = "serif"),
                            xlab  = gpar(fontfamily = "serif")))
...
To generate Figure 9, we began with creating a model using the initial approach (i.e the one that generated the President Trump Approval Score Model) using the same covariates as the MNAR model. Note that neither considered the survey weight, as there is no direct way to incorporate them into the non-ignorable framework. This highlights disagreements for the combined effect of Age and being Black and that of Age and being White.

There are multiple reasons for this discrepancy. Firstly, in parameterizing the second level of MNAR model, it isn't surprising that our demographic covariates would converge to slightly different values. Further, the poor representation among Asian, Hispanic/Latino, and those who identified as "Other" compounded instability in both modeling approaches. Another insight this discrepancy offers is that the effects of race/ethnicity on social distancing adherence are quite complex and require information from other available covariates to better understand the response.

For example, as we can see in Table 7 below, the distribution of President Trump Approval Score, for those participants with an available value, is highly dependent on the race/ethnic identity of the North Carolinian resident.

```{r, fig.align="center"}
covid.df %>%
  dplyr::rename(., Race.Ethnicity=Q19.20..Race...Ethnicity) %>%
  dplyr::group_by(Race.Ethnicity) %>%
  dplyr::summarize(`Mean Score` = round(mean(trump_approve_score, 
                                                na.rm=T),2),
                   .groups="drop") %>%
  pander(., "President Trump Approval Score Summary Across Racial/Ethnic Identities")
```

```{r, eval=F}
covid.df %>%
  dplyr::rename(., Children=Q5..Children.in.HH) %>%
  dplyr::group_by(Children) %>%
  dplyr::summarize(`Mean Score` = round(mean(trump_approve_score, 
                                                na.rm=T),2),
                   .groups="drop") %>%
  pander(., "President Trump Approval Score Summmary Across Number of Children in Household")
```
...
county.mod.table <- as.data.frame(summary(covid.df.county.mod)[[10]]) %>% 
  dplyr::select("Estimate", "Std. Error", "Pr(>|z|)")
...
# ## Age, Gender, Children, Race, President Trump, Age * Race
# with.age.gender.children.race.trump.intrxn.age.race <- with(data=imp.covid.df, 
#                          expr = glm(Social.Distancing.Adherence ~ DEMOGRAPHICS...GENDER +
#                                       Q5..Children.in.HH + 
#                                       Q19.20..Race...Ethnicity +
#                                       trump_approve_score +
#                                       age_race_ethnicity_asian + 
...
#                                     family=binomial,
#                                     weights=covid.weights))
# pool.age.gender.children.race.trump.intrxn.age.race <- pool(with.age.gender.children.race.trump.intrxn.age.race)
# covid.df.county.comparison.table <- data.frame(summary(pool.age.gender.children.race.trump.intrxn.age.race)) %>% 
#   dplyr::select("term", "estimate", "std.error", "p.value")
# colnames(covid.df.county.comparison.table) <- colnames(county.mod.table)
# rownames(covid.df.county.comparison.table) <- rownames(county.mod.table)
# covid.df.county.comparison.table$Variables <-county.mod.table$Variables
# covid.df.county.comparison.forest.table <- generate_forest_plot_df(covid.df.county.comparison.table)
# 
# forestplot(rownames(covid.df.county.comparison.forest.table[county.row, ]), 
#            mean  = cbind(covid.df.county.comparison.forest.table[county.row, ]$Estimate, 
#                          county.mod.forest.table$Estimate), 
#            lower = cbind(covid.df.county.comparison.forest.table[county.row, ]$Estimate - 
#                            1.96*covid.df.county.comparison.forest.table[county.row, ]$Std.Error, 
#                   county.mod.forest.table$Estimate - 
#                     1.96*county.mod.forest.table$Std.Error),
#            upper = cbind(covid.df.county.comparison.forest.table[county.row, ]$Estimate + 
#                            1.96*covid.df.county.comparison.forest.table[county.row, ]$Std.Error, 
#                   county.mod.forest.table$Estimate + 
#                     1.96*county.mod.forest.table$Std.Error),
#            line.margin = .2,
...
#            title="Figure 10: Final Model vs County Intercept Model",
#            txt_gp = fpTxtGp(title= gpar(fontfamily = "serif", fontface="plain",fontsize=10),
#                             label = gpar(fontfamily = "serif", fontface="plain",fontsize=10),
...
The coefficient estimates, confusion matrix, and residuals plot associated with this model are located in Table 13, Figure 20, and Figure 21 in the Appendix, respectively. As discussed in the methodology section, there are significant limitations to this model. Missing imputation was infeasible for this model, which limited the number of surveys and covariates. 

For those variables that are present in this model, the forest plot in Figure 10 compares the estimates and their precision to those of the final model. There appears to be generally wider confidence intervals for the random county intercept approach, which is unsurprising given the amount of surveys dropped in order to generate this model. In particular, there was little certainty surrounding most of the *Race/Ethnicity* groups.

Table 8 summarizes our estimate of $\tau^2$, which is the estimated variance of our random county intercepts, while Figure 22 in the Appendix displays a forest plot of the county intercepts. The F1-score of this model is approximately `r round(county.f1.score,3)`, and the false positive rate is `r round(county.false.positive,3)`. It is important to keep in mind that this slightly improved performance is incomprable to our final model, as it uses a completely different range of data. However, this model does present key insights into the abilities of politically and demographically contrasting counties to encourage social distancing.

Below are two heatmaps that depict estimated county intercepts (Figure 11a) and the 2020 President Trump vote share percentage (Figure 11b). In Figure 11a, a darker county corresponds to a higher intercept, and therefore a higher likelihood to socially distance. In Figure 11b, a darker county corresponds to a higher President Trump vote share percentage [[19]][Bibliography]. This data was sourced from politico.com and is listed in the Supplementary Links section. The indices on the figures reference specific counties we discuss in this section: Orange County (1), Durham County (2), Stokes County (3), and Davidson County (4). 

```{r north carolina heatmap, fig.width=5, fig.height=2.25, fig.align="center", warning=F,message=F,echo=FALSE,results='hide',fig.keep='all', cache=F}
map <- map_data("county")
...
nc_esda_plot <- ggplot(data=mapdata) + 
  labs(caption="") +
  geom_polygon(aes(x=long, 
                   y=lat, 
                   group = group, 
                   fill = 2*county_name..Intercept.-1),
  ...
  theme(legend.title = element_text(size = 8),
        ...
        plot.caption = element_text(hjust = 0.5, vjust = -0.5, size = 10),
...
```{r north carolina votes heatmap, fig.width=5, fig.height=4.5, fig.align="center", cache=F}
nc_votes_plot <- ggplot(data=mapdata) + 
  labs(caption="") +
  geom_polygon(aes(x=long, 
                   y=lat, 
                   group = group, 
...
grid.arrange(arrangeGrob(nc_esda_plot,
                         nc_votes_plot,
                         nrow=2, top = "Figures 11a and 11b"))
```
...
```{r, fig.width=9.5, fig.align="center", warning=F, message=F}
generate_model_agreement_df <- function(df, baseline.df, merge.df){
  df.vars <- rownames(df)
  
  df$Lower <- df$Estimate - 1.96 * df$Std.Error
  df$Upper <- df$Estimate + 1.96 * df$Std.Error
  
  baseline.df$Lower <- baseline.df$Estimate - 1.96 * baseline.df$Std.Error
  baseline.df$Upper <- baseline.df$Estimate + 1.96 * baseline.df$Std.Error
  
  df.agreement <- data.frame(baseline.sig = (0 < baseline.df[df.vars,]$Lower) | (0 > baseline.df[df.vars,]$Upper),
                                df.sig = 0 < df$Lower | 0 > df$Upper,
                                df.pt.in.baseline = df$Estimate >= baseline.df[df.vars,]$Lower &
                                  df$Estimate <= baseline.df[df.vars,]$Upper,
                                baseline.pt.in.df = baseline.df[df.vars,]$Estimate >= df$Lower &
                                  baseline.df[df.vars,]$Estimate <= df$Upper,
                                baseline.df.overlap = (baseline.df[df.vars,]$Upper >= df$Lower &
                                                         baseline.df[df.vars,]$Upper <= df$Upper) |
                                                         (baseline.df[df.vars,]$Lower >= df$Lower &
                                                            baseline.df[df.vars,]$Lower <= df$Upper) |
                                                         (df$Estimate >= baseline.df[df.vars,]$Lower &
                                                            df$Estimate <= baseline.df[df.vars,]$Lower)
                                )
  rownames(df.agreement) <- df.vars
  
  # -1 disagreement, -0.8 somewhat disagreement, 0.8 somewhat agreement, 1 agreement
  df.agreement <- df.agreement %>% 
    mutate(agreement=case_when(
      !baseline.sig & !df.sig ~ 1,
      baseline.pt.in.df & df.pt.in.baseline ~ 1,
      baseline.sig == df.sig & baseline.df.overlap ~ 0.8,
      baseline.sig != df.sig & baseline.df.overlap ~ -0.6,
      TRUE ~ -1
    ))
  
  merge(merge.df, data.frame(row.names=df.vars, 
                                agreement=df.agreement$agreement), 
        by=0, 
        all=TRUE) %>% 
    dplyr::select(Variables, agreement)
}

model.vs.baseline.list <- list(list(sensitivity.table, pres.trump.table),
                           list(mnar.final.model.table, mcar.model.mice.estimates.unweighted.table),
                           list(county.mod.table, pres.trump.table))

model.vs.baseline.results <- lapply(model.vs.baseline.list, 
                                    function(x) generate_model_agreement_df(df=x[[1]],
                                                                            baseline.df=x[[2]],
                                                                            merge.df=pres.trump.table)) %>% 
  bind_cols() %>%
  dplyr::select(Variables...1,
                starts_with("agreement"))
rownames(model.vs.baseline.results) <- model.vs.baseline.results$Variables
colnames(model.vs.baseline.results) <- c("Variables", "MAR", "MNAR", "County")
model.vs.baseline.results <- model.vs.baseline.results[!(row.names(model.vs.baseline.results)=="Age"), ]
model.vs.baseline.results <- model.vs.baseline.results[!(row.names(model.vs.baseline.results)=="Intercept"), ]
# There was an overlap of intervals for these, both were significant (did not capture 0), so they are shaded more lightly in the plot to indicate slightly stronger agreement relative to full disagreement in other variables
# model.vs.baseline.results["College Degree", ]$MAR = 0.8
# model.vs.baseline.results["White", ]$MAR = 0.8

model.vs.baseline.results.matrix <- as.matrix(model.vs.baseline.results %>%
                                                dplyr::select("MAR", "MNAR", "County"))

model.vs.baseline.results.matrix <- model.vs.baseline.results.matrix[c("Gender = Male",
                                   "College Degree",
                                   "Household Size",
                                   "Children in Household",
                                   "President Trump Approval Score",
...
rownames(model.vs.baseline.results.matrix) <- c("Gender",
                                   "College",
                                   "House",
                                   "Children",
                                   "Pr. Trump",
...
Next, we attempted a missing-not-at-random model by relating the missingness of *ESDA* to the value itself. The interpretations that resulted from this model were difficult to nail down conclusively, given its discrepancies with our other model estimates. However, the estimated value of the coefficient *b* that related missingness to *ESDA* suggests that the two are strongly related to one another. Thus, this offers evidence that our break-off assumption for our final model has some merit.
...
```{r , fig.width=9.5}
levelplot(model.vs.baseline.results.matrix,
          ylab="",
          xlab="",
          main=list(label="Figure 12: Model Agreement with President Trump Approval Score Model",
                    cex=0.8),
          par.settings=RdBuTheme(),
          colorkey=F)
```

Figure 12 highlights the greatest points of uncertainty among the models, relative to our final President Trump Approval Score model (for the MNAR model row, these comparisons reflect agreement with the unweighted version discussed in *Non-Ignorable Data Selection Model*). On the horizontal axis, *A*, *B*, *H/L*, *O* and *W* are abbreviations for *Asian*, *Black*, *Hispanic/Latino*, *Other*, and *White*, respectively. Note that "model agreement", which is represented with a dark blue shade, means either the coefficient estimates captured one another's point estimates and/or both intervals captured 0. For coefficient estimates with overlapping intervals of the same sign (i.e., strictly positive or negative intervals), the table cell is shaded light blue to represent partial agreement. Pink indicates that exactly one of the two models labeled the covariate as not significant, but the point estimates still overlapped in one another's intervals. Red indicates disagreement, and white indicates that the covariate was not present in the model of that row. 

As we can see, our greatest points of uncertainty surround the effects of age and race/ethnic identity, particularly among White, Black, and Hispanic/Latino residents. With respect to the county model, this discrepancy is likely a consequence of the fact that the model used a different range of data and accounted for locational information. With respect to the MNAR model, one possible reason for this is the disproportionate representation among different race/ethnic groups, as this could have led to multiple areas of convergence with a similar probability density.
...
cv_plot(with.age.children.race.gender.college.house.intrxn.age.race,
                       pool.age.children.race.gender.college.house.intrxn.age.race,
                       covid.df.regression,
                       create.cm.plot=TRUE,
                       subtitle.cm = "Figure 13: Demographics-Only Model on Imputation 1")[[3]]
```


```{r Demographics Residuals Plot, fig.height=3, fig.width=5,fig.align="center",warning=F,message=F,echo=FALSE,results='hide',fig.keep='all'}
...
arm::binnedplot(x=covid.df.demographic.preds,y=residuals(pooled.predict.demographic.obj, "pearson", scaled = TRUE),
                ...
                main = "Figure 14: Demographics Binned Residuals",
                cex.main=0.9, cex.lab=0.8)
```

\newpage

### President Trump Approval Score & Sensitivity Analysis
...
```{r survey mode model comparison, fig.align="center", fig.height=4}
## Age, Children, Race, Gender, College, House Size, Mode, Age x Race
with.age.children.race.gender.college.house.mode.trump.intrxn.age.race <- with(data=imp.covid.df, 
                                                     expr = glm(Social.Distancing.Adherence ~ 
                                                                  Q19.20..Race...Ethnicity +
                                                                  Q4..Number.of.People.in.HH +
                                                                  Q5..Children.in.HH + 
                                                                ...
                                                                family=binomial,
                                                                weights=covid.weights))
pool.age.children.race.gender.college.house.mode.trump.intrxn.age.race  <- pool(with.age.children.race.gender.college.house.mode.trump.intrxn.age.race)
table.age.children.race.gender.college.house.mode.trump.intrxn.age.race <-
  data.frame(summary(pool.age.children.race.gender.college.house.mode.trump.intrxn.age.race)) %>% 
  dplyr::select("term", "estimate", "std.error", "p.value")

survey.mode.table <- table.age.children.race.gender.college.house.mode.trump.intrxn.age.race
colnames(survey.mode.table) <- colnames(pres.trump.table)
survey.mode.table$Variables <-c(pres.trump.table$Variables, "Survey Mode == Live")
rownames(survey.mode.table) <- survey.mode.table$Variables
survey.mode.forest.table <- generate_forest_plot_df(survey.mode.table)

forestplot(rownames(pres.trump.forest.table), 
           mean  = cbind(pres.trump.forest.table$Estimate, survey.mode.forest.table[rownames(pres.trump.forest.table),]$Estimate), 
           lower = cbind(pres.trump.forest.table$Estimate - 1.96*pres.trump.forest.table$Std.Error, 
                  survey.mode.forest.table[rownames(pres.trump.forest.table),]$Estimate - 1.96*survey.mode.forest.table[rownames(pres.trump.forest.table),]$Std.Error),
           upper = cbind(pres.trump.forest.table$Estimate + 1.96*pres.trump.forest.table$Std.Error, 
                  survey.mode.forest.table[rownames(pres.trump.forest.table),]$Estimate + 1.96*survey.mode.forest.table[rownames(pres.trump.forest.table),]$Std.Error),
           line.margin = .2,
           col = fpColors(box = c("blue", "darkred"),
                          line = c("blue", "darkred")),
           legend = c("Final", "Final + Survey Mode"),
           title="Figure 15: Final Model vs Final Model with Survey Mode",
           txt_gp = fpTxtGp(title= gpar(fontfamily = "serif", fontface="plain", fontsize=10),
                            label = gpar(fontfamily = "serif", fontface="plain", fontsize=10),
                            ticks = gpar(fontfamily = "serif"),
                            xlab  = gpar(fontfamily = "serif")))
...
```{r Residuals trump model, fig.height=2.8, fig.width=4.5,fig.align="center",warning=F,message=F,echo=FALSE,results='hide',fig.keep='all'}
...
arm::binnedplot(x=covid.df.trump.preds,y=residuals(pooled.predict.trump.obj, "pearson", scaled = TRUE),
                xlab="Predicted Probabilities", 
                ylab="Binned Residuals",
                main = "Figure 16: President Trump Binned Residuals",
                cex.main=0.9, cex.lab=0.8)
```
...
pander(sensitivity.table, "MAR Model")
...
pander(complete.case.table, "Complete Case Model")
```

```{r forestplot complete case analysis model, message=FALSE, fig.height=3.5, fig.align="center"}
...
forestplot(rownames(pres.trump.forest.table), 
           ...
           title="Figure 17: Final Model vs Complete Case Model",
           txt_gp = fpTxtGp(title= gpar(fontfamily = "serif", fontface="plain", fontsize=8),
                            label = gpar(fontfamily = "serif", fontface="plain", fontsize=8),
                            ticks = gpar(fontfamily = "serif"),
                            xlab  = gpar(fontfamily = "serif")))
```

```{r remaining traceplots, fig.align="center", fig.height=6}
tidybayes::gather_draws(mnar.final.model %>% as.mcmc(.), 
                        alpha, 
                        beta1, 
                        beta2, 
  ...
  mutate(variable= case_when(
    ...
    `.variable` == "beta1" ~ "beta[1]",
    `.variable` == "beta2" ~ "beta[21]",
    `.variable` == "beta3" ~ "beta[22]",
    `.variable` == "beta4" ~ "beta[23]",
    `.variable` == "beta5" ~ "beta[24]",
    `.variable` == "beta6" ~ "beta[30]",
    `.variable` == "beta7" ~ "beta[31]",
    `.variable` == "beta8" ~ "beta[32]",
    `.variable` == "beta9" ~ "beta[33]",
    `.variable` == "beta10" ~ "beta[34]"
  )) %>%
  ggplot() + 
  geom_line(aes(x = .iteration, y = .value, color = as.factor(.chain)), alpha = 0.7) + 
  facet_grid(variable~., scales = "free_y", labeller = label_parsed) + 
  labs(title ="Figures 18 and 19: Traceplots and ACF Plots", 
       x = "Iteration", 
       y = "Value", 
       color = "Chain") +
  theme(legend.position="none",
...
```{r remaining acf plots, message=F, warning=F, fig.height=4, fig.width=7, fig.align="center"}
plot_acf <- function(parameter, mod, show.y=FALSE) {
  acf.p <- mod$BUGSoutput$sims.matrix %>% 
    as_tibble() %>% 
    pull(parameter)
  acf(acf.p, main=parameter, xlab="", ylab="") 
}
par(mfrow = c(1, 2),oma=c(0,0,1,0))
invisible(sapply(c("a", "b"), 
                 function(x) plot_acf(x, mnar.final.model)))
```
...
plot_confusion_matrix(county.cm.tibble, 
                              add_row_percentages = FALSE,
                              add_col_percentages = FALSE) + 
  ggplot2::labs(subtitle = "Figure 20: Random County Intercept Model") + 
  ggplot2::theme(plot.subtitle = element_text(hjust = 0.5, size = 10))
```

```{r Residuals county model, fig.height=3, fig.width=5,fig.align="center",warning=F,message=F,echo=FALSE,results='hide',fig.keep='all'}

arm::binnedplot(x=rand.county.predictions,y=residuals(covid.df.county.mod, "pearson", scaled = TRUE),
                xlab="Predicted Probabilities", 
                ylab="Binned Residuals",
                main = "Figure 21: Random County Binned Residuals",
                cex.main=0.9, cex.lab=0.8)
```

```{r random effects plot for counties, fig.height=4, fig.align="center", warning=F, message=F}
random.effects.plot <- plot_model(covid.df.county.mod, type = "re", facet.grid=FALSE, title="Figure 22: Random Effects over Counties") 
