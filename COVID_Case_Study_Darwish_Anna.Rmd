...
a) **Trump_approval_score**: This ranking indicates the estimated respondents' level of approval of Trump's COVID policies from a scale of 1-5 and was predicted by Clarity Campaigns. The prediction was based on voter file records for respondents who were registered to vote, and is the only covariate included that was not contained within the actual survey itself.
...
```{r Basic Stepwise Logistic Regression Model, fig.align="left",warning=FALSE,echo=FALSE, cache=T}
...
covariate.summary[1, 1] <- substr(covariate.summary[1, 1], 1, nchar(covariate.summary[1, 1])-3)
covariate.summary[4, 1] <- substr(covariate.summary[4, 1], 1, nchar(covariate.summary[4, 1])-3)
covariate.summary[6, 1] <- substr(covariate.summary[6, 1], 1, nchar(covariate.summary[6, 1])-3)
covariate.summary <- covariate.summary[order(-covariate.summary$Imputations),]
rownames(covariate.summary)  <- covariate.summary$Covariates

appended.frame <- data.frame(Imputations=c(0,0,0,0))
rownames(appended.frame) = c("Age + Children + Race/Ethnicity","Age * Chilren + Race", "Age * Race + Children", "Children * Race + Age")
appended.frame$Covariates = rownames(appended.frame)
covariate.summary  <- rbind(covariate.summary,appended.frame)
covariate.summary  <- covariate.summary %>% dplyr::select("Imputations")
```
...
Table 2 summarizes the different models I explored for the demographics-only model, the number of imputations they appeared post-variable selection with lasso regression, and their F1-scores and false postive rates. Recall that in this context, a false positive is classifying someone as social distancing when they are in fact not socially distancing. As you can tell from the false positive rates, all of these models were predisposed to committing this error, while our final model performed best in terms of both metrics.
...
#                 main = "Figure 4: Demographics-Only Binned Residuals",
...
From Table 4, we can see that participants with either one or 3+ children did not significantly differ from participants of the same age and race/ethnic identity and no children. However, holding all else constant, we are 95% confident that the odds a participant with two children is socially distancing is between `r round(exp(demographics.table["Q5..Children.in.HH2","estimate"]- 1.96 * demographics.table["Q5..Children.in.HH2","std.error"]),3)` and `r round(exp(demographics.table["Q5..Children.in.HH2","estimate"]+ 1.96 * demographics.table["Q5..Children.in.HH2","std.error"]),3)` times more likely than those with no children. A possible reason why this co-variate is significant, while the one for 3+ children is not is the limited number of participants in the dataset who have 3+ children. This makes it difficult to estimate the covariate.

Because of the interaction term between age and race/ethnic identity, it is slightly more complex to interpret these terms. We will accomplish this by considering NC residents of the mean-centered age, which was 48.6 years old. All of the following interpretations will be made relative to an individual of this age. If we consider our baseline race/ethnic identity, we are 95% confident that for every decade increase in age, the odds the elder Asian NC resident is social distancing is between `r round(exp(10*demographics.table["age","estimate"]- 1.96 * demographics.table["age","std.error"]),3)` and `r round(exp(10*demographics.table["age","estimate"] + 1.96 * demographics.table["age","std.error"]),3)` times that of a younger Asian NC resident, holding all else constant. We estimate that for every decade increase in age, the odds an older Black NC resident is social distancing is `r round(exp(10*demographics.table["age","estimate"]+10*demographics.table["age:Q19.20..Race...EthnicityBlack","estimate"]),3)` times that of a younger Black NC resident, holding all else constant. We estimate that for every decade increase in age, the odds an older Hispanic/Latino NC resident is social distancing is `r round(exp(10*demographics.table["age","estimate"]+10*demographics.table["age:Q19.20..Race...EthnicityHispanic.Latino","estimate"]),3)` times that of a younger Hispanic/Latino NC resident, holding all else constant. We estimate that for every decade increase in age, the odds an older NC resident who selected "Other" for their racial/ethnic identity is social distancing is `r round(exp(10*demographics.table["age","estimate"]+10*demographics.table["age:Q19.20..Race...EthnicityOther","estimate"]),3)` times that of their younger counterpart. Finally, we estimate that for every decade increase in age, the odds an older White NC resident is social distancing is `r round(exp(10*demographics.table["age","estimate"]+10*demographics.table["age:Q19.20..Race...EthnicityWhite","estimate"]),3)` times that of their younger counterpart.
...
The results of this model indicate that after accounting for an NC's resident's age, number of children, and race/ethnic identity, Trump approval score is not significant. A possible explanation for this could be that political beliefs have strong relationships with race/ethnicity and age [CITE]. Further, the trustworthiness of the Trump approval score co-variate was always unclear given that the question was not directly asked during the survey.

## Random County Intercept Model

```{r Random Intercept , fig.align = "center",warning=FALSE,echo=FALSE,cache=T}
covid.df.county <- covid.df.original[!is.na(covid.df.imp.county$county_name), ]
covid.df.county <- covid.df.county[!is.na(covid.df.county$Q5..Children.in.HH), ]
covid.df.county <- covid.df.county[!is.na(covid.df.county$Social.Distancing.Adherence), ]

covid.df.county.mod <- glmer(Social.Distancing.Adherence ~ Q5..Children.in.HH + age * Q19.20..Race...Ethnicity + (1|county_name), 
                              family=binomial,
                              weights=covid.df.county$weight,
...
```{r Random Intercept Table Tau, fig.align = "center",warning=FALSE,echo=FALSE,cache=T}
...
colnames(table.age.children.race.intrxn.age.race.display) <- c("Est", "StdErr", "PVal")
...
covid.df.sensitivity.df <- mice(covid.df.sensitivity, m=3, print=F, maxit = 40, seed=2525)

if (file.exists("sensitivity_imp/sensitivity_imp.Rdata")) {
  covid.df.sensitivity.df <- miceadds::load.Rdata("sensitivity_imp/sensitivity_imp.Rdata", "covid.df.sensitivity.df")
} else {
  covid.df.sensitivity.df <- mice(covid.df.sensitivity.df, m=3, print=F, maxit = 40, seed=2525)
  miceadds::write.mice.imputation(covid.df.sensitivity.df, "sensitivity_imp", include.varnames=TRUE)
}

curr.sensitivity.df <- complete(covid.df.sensitivity.df,1)
curr.sensitivity.df$Social.Distancing.Adherence <- rep(TRUE,nrow(covid.df.benefit))
curr.sensitivity.df <- curr.sensitivity.df %>% mutate(Social.Distancing.Adherence = case_when(
          !is.na(Q6..Non.HH.Face.to.Face.Count) &  Q6..Non.HH.Face.to.Face.Count == 0 ~ TRUE,
          !is.na(Q6..Non.HH.Face.to.Face.Count) &
            !is.na(Q7..Six.Feet.Away...If.Q6...0.) &
            Q7..Six.Feet.Away...If.Q6...0. -  Q6..Non.HH.Face.to.Face.Count != 0 ~ FALSE,
...
res <- glm(Social.Distancing.Adherence ~ age * Q19.20..Race...Ethnicity + Q5..Children.in.HH, family=binomial,data=curr.sensitivity.df)
res <- data.frame(coef(summary(res))) %>% dplyr::select("Estimate", "Std..Error", "Pr...z..")
colnames(res) <- c("Est", "StdErr", "PVal")
...
colnames(table.trump.age.children.race.intrxn.age.race) <-  c("Est", "StdErr", "PVal")
...
county.results.table.appendix <- data.frame(Estimates=summary(covid.df.county.mod)$coefficients[, 1], 
                                       Std.Err=summary(covid.df.county.mod)$coefficients[, 2], 
                                       P.Value=summary(covid.df.county.mod)$coefficients[, 4])
