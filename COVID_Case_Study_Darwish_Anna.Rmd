...
Table 2 presents summary statistics for the aforementioned demographic co-variates across the 4293 completed surveys with associated voter registration results. While this summary is not representative of all the participants, it underscores the difference between the evaluated and self-reported social distancing adherence, prior to making any assumption regarding the breakoff point. Additionally, as we can see from the mean age and racial/ethnic identity breakdown, older and White participants are disproportionately represented [[14]][Bibliography]
...
complete.case.race.ethnicity <- paste0(100*round(sum(cc.covid.df$Q19.20..Race...Ethnicity == "Asian")/nrow(cc.covid.df),2),
                                      "% Asian, ",
                                      100*round(sum(cc.covid.df$Q19.20..Race...Ethnicity == "Black")/nrow(cc.covid.df),2),
                                      "% Black, ",
                                      100*round(sum(cc.covid.df$Q19.20..Race...Ethnicity == "Hispanic.Latino")/nrow(cc.covid.df),2),
                                      "% Hispanic/Latino, ",
                                      100*round(sum(cc.covid.df$Q19.20..Race...Ethnicity == "Other")/nrow(cc.covid.df),2),
                                      "% Other, ",
                                      100*round(sum(cc.covid.df$Q19.20..Race...Ethnicity == "White")/nrow(cc.covid.df),2),
                                      "% White")
...
complete.case.summary <- data.frame(Metric=c("Mean Age",
                                    ...
                                    Summary=c(paste0(round(mean(cc.covid.df$age),2), " Years"),
                                              paste0(round(mean(cc.covid.df$Q1..Health.Quality),2)),
...
In addition to the sensitivity analysis involved in creating models across 25 different imputations, I also recreated the final President Trump Approval Score model using the *mice* package to impute values for our response prior to incorporating the break-off point assumption. This is a classical MAR approach, as we use the available co-variates to populate the response values. In order to establish a baseline for expected performance, I also recreated this final model via a complete case approach. The coefficient estimates of the final model, the sensitivity model, and a complete case approach model are located in Tables 7, 8, and 9, respectively. Random forest plots comparing the latter two models to the final one are located in Figures 3 and 10, respectively.

While the final model arguably incorporates missingness information into the response, it relies on a strong assumption with respect to the nature of the missingness. For this reason, I also created a hierarchical Bayesian model, where I first use the complete demographic covariates (age, race/ethnicity, and gender) to predict the likelihood that the *ESDA* of the surveyee is missing. The next level then relates that likelihood to the likelihood that the surveyee is social distancing. 

Due to the constrained range of the response for both steps in the hierarchy, I relied on relatively flat priors for all of the parameters. While prior selection built on previous literature is often a key component of Bayesian modeling, the novelty of pandemic behaviors did not sufficiently align with existing literature on adherence to other official health recommendations. One potential extension of the work done in this paper would be to examine how trends here relate to a given resident's likelihood to get the vaccine.

The parametrization of this model is:

$$
\begin{aligned}
log(\frac{M_{i}}{1-M_{i}}) = \beta_0 + \beta_1 * I(Sex_i = Male) + \beta_2 * Health \ + \\
 \sum_{r=1}^4 \beta_{3r} * I(Race.Ethnicity_i = r) \ + \sum_{r=0}^4 \beta_{4r} * Age_i * I(Race.Ethnicity_i = r)
 \\
 log(\frac{P_{i}}{1-P_{i}}) = \alpha_0 + \alpha_1 * M_i
 \\
 \beta_i \sim N(0,1) \\
 \alpha_i \sim N(0,1)
\end{aligned}
...
As discussed in the *Methods* section, the following model subsumes our final demographics-only model and accounts for President Trump Approval Score. We will also interpret the demographic covariate estimates per this fuller model.

## President Trump Approval Score and Demographics Model
...
imputation.1.results <- plot_confusion_matrix(complete.case.cm, 
                      add_row_percentages = FALSE,
                      add_col_percentages = FALSE) +
      ggplot2::labs(subtitle = "Complete Case Approach - Imputation 1") +
      ggplot2::theme(plot.subtitle = element_text(hjust = 0.5, size = 10))
```
...
complete.case.results <- plot_confusion_matrix(table1, 
                      add_row_percentages = FALSE,
                      add_col_percentages = FALSE) +
      ggplot2::labs(subtitle = "Complete Case Approach: Non-Missing Data") +
      ggplot2::theme(plot.subtitle = element_text(hjust = 0.5, size = 10))
...
covid.df.county.mod <- glmer(Social.Distancing.Adherence ~ Q5..Children.in.HH + 
                               Q19.20..Race...Ethnicity + 
                               trump_approve_score +  
                               age_race_ethnicity_asian + 
                               age_race_ethnicity_black +
...
For those variables that are present in this model, the forest plot in Figure 5 compares the estimates and their precision to the final model. There appears to be generally wider confidence intervals for the random county intercept approach, which is unsurprising given the amount of surveys dropped in order to generate this model. In particular, there was little certainty surrounding most of the *Race/Ethnicity* groups, which is likely a consequence of having to drop residents without a voter registration file.

Table 5 summarizes our estimate of $\tau_0^2$, which is the estimated variance of our random county intercepts, while Figure 13 in the Appendix displays a forest plot of the county intercepts. The F1-score of this model is approximately `r round(county.f1.score,3)`, and the false positive rate is `r round(county.false.positive,3)`. While this model arguably has slightly improved performance relative to our final model, it uses a completely different range of data and can not be compared with these metrics. However, the natural heterogeneity across counties, especially in a state as politically split as North Carolina, offers key insights into the ability of different political and demographic communities to respond in a manner that encourages social distancing.

Below are two heatmaps that depict these estimated intercepts (Figure 6a) and the 2020 President Trump vote share percentage (Figure 6b). In Figure 6a, a darker county corresponds to a higher intercept, and therefore a higher likelihood to socially distance. In Figure 6b, a darker county corresponds to a higher President Trump vote share percentage [[19]][Bibliography]. This data was sourced from politico.com and is listed in the Supplementary Links section. The indices on the figures reference specific counties we discuss in this section: Orange County (1), Durham County (2), Stokes County (3), and Davidson County (4). 
...
Another limitation to this analysis is that the final model does a rather poor job of fitting our data. Given some discrepancies with the final model's estimates and other studies discussed, the reliability of our estimates and interpretations is unclear. This is largely due to the homogeneity of the response variable, *ESDA*, which raised additional concerns throughout this project.
