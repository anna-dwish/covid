...
$$
\begin{aligned}
log(\frac{M_{i}}{1-M_{i}}) = \alpha + \beta_1 * I(Sex_i = Male) +\sum_{r=1}^4 \beta_{3r} * I(Race.Ethnicity_i = r) \ + \\
...
Figure 5 displays traceplots for our final model's a and b parameter chains. The random scatter around the mean estimate, along with clear "mixing" between the different chains, indicate convergence for these parameters. Please reference Figure INSERT in the Appendix for the remaining traceplots for our final model. Similar plots were generated for other prior combinations that were examined to ensure convergence prior to comparison.

Figures INSERT in the Appendix contain our auto-correlation plots, where each peak at the beginning and quickly drop close to 0 as the lag increases. This provides evidence that the iterations sufficiently expxlored the parameter space, as they were not strongly dependent upon one another.
...
In Figure 6, we explore the posterior distributions of $a$, the $b$ across different priors on the demographic co-variates, along with $a$ and $b$ themselves. In both figures, the density plot with a dashed line is the posterior density plot of our final model. Given that our posterior estimate for both parameters roughly fall at the peaks for other priors as well, we have sufficient evidence that our parameters are insensitive to prior choice. Please refer to Figures INSERT for the remaining prior sensitivity plots on the demographic covariate terms, and note that each model had the same prior $N(0,1)$ prior on the intercept $\alpha$.
...
Figure 7 summarizes a forestplot comparing the MNAR model's estimates to the one generated using the same covariates (age, gender, and race/ethnic identity) with the first modeling approach. This highlights key disagreements among all of the racial/ethnic estimates, except for those who identified as "Other". Specifically, the final model estimated that the combined effect of Age and being White was slightly positive, as was that of Age and being Black. In contrast, this model estiamtes these combined effects to be negative. Even more surprising, the MNAR model is more consistent with our exploratory data analysis (see Figure 1) than the *mice* model is. 
...
covid.df %>%
  ...
  dplyr::summarize(`Mean Score` = round(mean(trump_approve_score, 
                                                na.rm=T),2),
                   .groups="drop") %>%
  pander(., "President Trump Approval Score Summmary Across Racial/Ethnicity Groups")
```
...
tidybayes::gather_draws(mnar.final.model %>% as.mcmc(.), 
  ...
  geom_line(aes(x = .iteration, y = .value, color = as.factor(.chain)), alpha = 0.7) + 
  facet_grid(.variable~., scales = "free_y") + 
  labs(title = bquote(atop(paste("Traceplots of ",
                                alpha, ", ", 
                                beta[1], ": Gender=Male, ", 
                                beta[2], ": R/E=Asian, ",
                                beta[3], ": R/E=Black, ",
                                beta[4], ": R/E=Hispanic/Latino, ",
                                beta[5], ": R/E=Other, "),
                           paste(
                                beta[6], ": Age x R/E=Asian, ",
                                beta[7], ": Age x R/E=Black, ",
                                beta[8], ": Age x R/E=Hispanic/Latino, ",
                                beta[9], ": Age x R/E=Other, ",
                                beta[10], ": Age x R/E=White"))), 
       x = "Iteration", y = "Value", color = "Chain") +
  theme(plot.title = element_text(size=8))
```

```{r remaining acf plots, message=F, warning=F, fig.height=5, fig.align="center"}
